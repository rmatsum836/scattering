...
def compute_partial_van_hove(trj, chunk_length=10, selection1=None, selection2=None,
    ...
    #    if pair[0] in trj.topology.select(selection1) and pair[1] in trj.topology.select(selection1):
    #        remove_list.append(i)
    #    elif pair[0] in trj.topology.select(selection2) and pair[1] in trj.topology.select(selection2):
    ...
    g_r_t = None
    for index,i in enumerate(range(n_chunks)):
        print(index)
        times = list()
...
def compute_van_hove_il(trj, chunk_length, water=False,
                     r_range=(0, 1.0), bin_width=0.005, n_bins=None,
                     periodic=True, opt=True):
    """Compute the partial van Hove function of a trajectory
    ...
    #unique_elements = list(set([[a.element,a.residue.name] for a in trj.top.atoms]))
    #unique_elements = [{a.residue.name: a.element} for a in trj.top.atoms]
    #unique_elements = [dict(y) for y in set(tuple(x.items()) for x in unique_elements)]
    #unique_resnames = list(set([a.residue.name for a in trj.top.atoms]))
    names = list(set([i.name for i in trj.topology.atoms]))
    elements = list(set([i.element for i in trj.topology.atoms]))
    element_dict = dict()
    for name in names:
        for element in elements:
            if element.symbol in name:
                element_dict[name] = element.symbol
    ...
    for elem1, elem2 in it.combinations_with_replacement(names[::-1], 2):
        ...
        concentration1 = trj.atom_slice(trj.top.select('name {}'.format(elem1))).n_atoms / trj.n_atoms
        concentration2 = trj.atom_slice(trj.top.select('name {}'.format(elem2))).n_atoms / trj.n_atoms
        form_factor1 = get_form_factor(element_name=element_dict[elem1], water=water)
        form_factor2 = get_form_factor(element_name=element_dict[elem2], water=water)
